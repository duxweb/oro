...
`Update` writes normal model fields and compares `ParentID` with the current database row. If `ParentID` changed, the subtree is moved and `_lft`, `_rgt`, and `Depth` are recalculated.

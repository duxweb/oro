...
`Define` receives a `SchemaBuilder`. Use it to set the table, connection, fields, indexes, relations, tenant, and shard metadata.
...
| `Tenant(fields...)` | model tenant fields |
| `NoTenant()` | opt out of tenancy |

...
Bulk upsert returns affected rows:
...
For bulk upsert, `Update` / `UpdateAll` use each row's own inserted values. Use `UpdateMap` when the conflict update should be a shared constant or expression for the whole statement.

...
`Table(...).MapTo[T]()` supports `First`, `Get`, `Stream`, `Chunk`, `Each`, `Paginate`, `Create`, and `CreateManyResult`. `UpsertMany` returns affected rows, so it does not map DTO values.

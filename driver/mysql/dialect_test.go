...
func TestCompileSelectOffsetWithoutLimit(t *testing.T) {
	...
	want := "select * from `products` limit 18446744073709551615 offset 10"
	if sql.SQL != want {
		t.Fatalf("got SQL %q, want %q", sql.SQL, want)
	}
}
...
func TestCompileSelectJSONLikeCondition(t *testing.T) {
	...
	want := "select * from `products` where json_unquote(json_extract(`meta`, ?)) like ?"
	if sql.SQL != want {
		t.Fatalf("got SQL %q, want %q", sql.SQL, want)
	}
...
func TestCompileSchemaUUIDPrimaryKeyDoesNotAutoincrement(t *testing.T) {
	...
	want := "create table if not exists `users` (`id` char(36) primary key)"
...
func TestCompileSelectJSONCondition(t *testing.T) {
	...
	want := "select * from `products` where json_extract(`meta`, ?) = cast(? as json) and json_contains_path(`meta`, 'one', ?) = 1 and json_extract(`meta`, ?) = cast(? as json)"
	if sql.SQL != want {
		t.Fatalf("got SQL %q, want %q", sql.SQL, want)
	}
...
func TestCompileUpsert(t *testing.T) {
	...
	want := "insert into `products` (`code`, `price`) values (?, ?) on duplicate key update `price` = VALUES(`price`)"
	if sql.SQL != want {
		t.Fatalf("got SQL %q, want %q", sql.SQL, want)
	}
	if len(sql.Args) != 2 || sql.Args[0] != "A001" || sql.Args[1] != 100 {
